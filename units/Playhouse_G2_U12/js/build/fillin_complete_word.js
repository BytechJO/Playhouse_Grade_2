function buildFillInBody(aObj) {
  ...
  if (typeof aObj != undefined && aObj != null) {
    ...
    for (x = 0; x < numOfQuestions; x++) {
      ...
      if (tmpObj.writeFullWord === true) {
        htmlStmt += '<div class="full_word_input_holder">';

        htmlStmt +=
          '<input class="text_input_area full_word_input" ' +
          'type="text" ' +
          'maxlength="' +
          (tmpObj.fullWordMaxlength || 20) +
          '" ' +
          'data-type="text">';
...
function enableAutoMoveToNextInput() {
  $(".activity_area").on("input", ".text_input_area", function () {
    var currentInput = $(this);

    var maxLength = parseInt(currentInput.attr("maxlength"), 10);

    /*
     * ينقل فقط إذا امتلأ الإنـبوت.
     * إنبوت الكلمة الكاملة maxlength أكبر من 1،
     * لذلك لن ينتقل بعد أول حرف.
     */
    if (currentInput.val().length >= maxLength) {
      var question = currentInput.closest(".que");

      var inputs = question.find(".text_input_area");
  ...
  /*
   * عند الضغط على Backspace داخل إنبوت فارغ،
   * يرجع للإنبوت السابق.
   */
  $(".activity_area").on("keydown", ".text_input_area", function (event) {
