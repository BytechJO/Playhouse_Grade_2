...
function getCurrFileOrDirectory(aNeed, aPath) {
  var href =
    aPath === undefined || aPath === null ? window.location.href : aPath;
  ...
  if (href !== null && href !== undefined) {
    ...
    // إزالة / من آخر الرابط إذا موجودة
    href = href.replace(/\/+$/, "");

    lastPathSegment = href.substring(href.lastIndexOf("/") + 1);

    var path = href.substring(0, href.lastIndexOf("/"));
    var pathArr = path.split("/");

    currFolder =
      pathArr.length > 0 ? pathArr[pathArr.length - 1].toLowerCase() : "";

    if (currFolder === "views") {
      unitFolder = pathArr.length > 1 ? pathArr[pathArr.length - 2] : "";
    } else {
  ...
  /*
      Netlify Pretty URL قد يعيد:
      slide_images

      بينما _activityData يحتوي:
      slide_images.html

      لذلك نعيد الامتداد عند قراءة اسم الملف داخل views.
    */
  if (
    aNeed === "file" &&
    currFolder === "views" &&
    lastPathSegment.indexOf(".") === -1
  ) {
