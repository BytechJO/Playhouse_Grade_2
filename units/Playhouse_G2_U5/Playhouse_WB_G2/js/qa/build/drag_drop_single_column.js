function initActivity(activity){
	...
	//Questions
	drag_drop_questions = '<div class="drag_drop_questions d-flex flex-wrap center_item"><ul class="container">';
	jQuery.each(activity.questions, function(key, values){
		...
		if(typeof(values)=="string"){
			...
			jQuery.each(values, function(k, v){
				var v = v+"";
				drag_drop_questions += '<li class="drag_drop_multiple" style="margin:0px">'+ v.replace('___', ' <input readonly type="text" class="droppable_div" />') +'</li>';
			});
		}
