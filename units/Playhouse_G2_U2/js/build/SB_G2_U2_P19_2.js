		...
		for (x = 0; x < numOfRows; x++) {
			...
			for (y = 0; y < numInRowArray[x].length; y++) {
				...
				// if (aObj.defaultAnswer == currentQue) {
					...
					var imagesText = ((aObj.questions)[currentQue - 1]).imagesText;
					...
					if(imagesText !="" && imagesText != undefined){
						for(let i=0; i<imagesText.length; i++){
							htmlStmt += '<div class="audioIcon txt-audioIcon imagesText d-flex off contant min_w_fit_contant" data-audio="' + ((aObj.questions)[currentQue - 1]).audio + '">';
							htmlStmt += '<img src="'+imagesText[i]+'">';
							htmlStmt += '</div>';
						}
					}
