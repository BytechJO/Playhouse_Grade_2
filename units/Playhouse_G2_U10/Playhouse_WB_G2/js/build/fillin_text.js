function buildFillInBody(aObj) {
  ...
  /* =========================================================
   Allow big/scary in either order for questions 1 and 2
========================================================= */

  var $firstInput = $('.que[data-qno="1"] input.text_input_area');
  var $secondInput = $('.que[data-qno="2"] input.text_input_area');

  function updateFirstSentenceAnswers() {
    var firstValue = $.trim($firstInput.val()).toLowerCase();
    var secondValue = $.trim($secondInput.val()).toLowerCase();

    if (firstValue === "scary" && secondValue === "big") {
      // scary and big
      aObj.questions[0].answer = ["scary"];
      aObj.questions[1].answer = ["big"];
    } else if (firstValue === "big" && secondValue === "scary") {
      // big and scary
      aObj.questions[0].answer = ["big"];
      aObj.questions[1].answer = ["scary"];
    } else {
      /*
      أي تركيب آخر سيبقى خطأ، ومن ضمنه:
      big and big
      scary and scary
    */
      aObj.questions[0].answer = ["__invalid_first__"];
      aObj.questions[1].answer = ["__invalid_second__"];
    }

    aObj.questions[0].alternateanswer = [[]];
    aObj.questions[1].alternateanswer = [[]];
  }

  $firstInput
    .add($secondInput)
    .off("input.firstSentenceAnswers")
    .on("input.firstSentenceAnswers", function () {
      updateFirstSentenceAnswers();
    });
