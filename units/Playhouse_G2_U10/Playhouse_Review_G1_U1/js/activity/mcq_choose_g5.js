...
MCQ.prototype = {
    ...
    if (ques.length > 0) {
      ...
      for (var i = 0; i < picks.length; i++) {
        picks[i].addEventListener("click", function () {
          ...
          thisPickSet = $("#" + _thisId).closest(
            "#pick_set_" + _parentNum + "_" + _parentPickSet,
          );
  ...
  validate: function () {
    ...
    $area.find(".que").each(function () {
      ...
      var qIndx = parseInt($question.data("qno"));
      ...
      $question.find(".pick_set").each(function () {
        var $pickSet = $(this);

        var setIndx = parseInt($pickSet.attr("id").split("_")[3]);
        ...
        $pickSet.find(".selected").each(function () {
          var selectedIndex = parseInt($(this).attr("id").split("_")[3]);
        ...
        var correctAnswers = _cAns.slice();
      ...
      /* =========================================
           ...
           مهم:
           نقارن بعدد الـ pick_set
           وليس بعدد عناصر answer
        ========================================= */

      var numberOfSets = $question.find(".pick_set").length;

      var numberOfCorrectSets = 0;

      for (var r = 0; r < tmpRes.length; r++) {
        if (tmpRes[r] === 1) {
          numberOfCorrectSets++;
        }
      }

      if (numberOfSets > 0 && numberOfCorrectSets === numberOfSets) {
        resultArr[qIndx - 1] = 1;
      } else {
        resultArr[qIndx - 1] = 0;
      }
      ...
      if (resultArr[qIndx - 1] == 1) {
        if (fDataObj.audio != "" && fDataObj.audio != "no") {
          if (fDataObj.audioenable == "correct") {
            if ($question.find(".audioIcon").length > 0) {
              $question.find(".audioIcon").removeClass("disabled");
            }
          }
        }
      }
    ...
    allCorrect = resultArr.length > 0 && resultArr.indexOf(0) === -1;
