        ...
        $area.find('.que').each(function(){
            ...
            $(this).find('.pick_set').each(function(){
                ...
                if($(this).find('.selected').length == 1){
                    ...
                    if(selectedPick == _cAns){
                        tmpRes[setIndx-1] = 1;
                    }
                } else if($(this).find('.selected').length > 1){
                    var tSel = $(this).find('.selected');
                    var allAns = []
                    for (x = 0; x < tSel.length; x++){
                        currElem = tSel[x]
                        allAns.push(parseInt(currElem.id.split('_')[3]));
                    }
                    console.log('pick_set >> ',allAns)
                    selectedPick = allAns

                    if(selectedPick.every((value, index) => value === _cAns[index])){
                        tmpRes[setIndx-1] = 1;
                    }
                }
                console.log('pick_set >> ',$(this).attr('id'), _cAns, setIndx, _setCorrAns, selectedPick);
