...
LineDraw.prototype = {
  ...
  setValuesForLine: function (lineOb) {
    ...
    lineOb.endY =
      ...
      lineHalfHeight;
    // temporary put + 20 --> should be fixed...
    lineOb.endX =
      lineOb.endNode.position().left + (ed_margin_left - halfWidth) + 20;
  ...
  showIcons: function (aBoo, aResult) {
    ...
    $area.find(".drop").each(function () {
      ...
      if (aBoo) {
        ...
        // مهم:
        // امسح نتيجة الـ Check السابقة أولاً
        $tick.css("display", "none");
        $cross.css("display", "none");

        // بعدين اعرض النتيجة الجديدة فقط
