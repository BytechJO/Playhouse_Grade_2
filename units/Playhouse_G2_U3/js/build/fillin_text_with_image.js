function buildFillInBody(aObj) {
  ...
  if (typeof aObj != undefined && aObj != null) {
    ...
    if (aObj.image != undefined && aObj.image != "") {
      ...
      for (let y = 0; y < aObj.image.length; y++) {
        htmlStmt += '<img src="' + aObj.image[y] + '"/>';
      }
      htmlStmt += "</div>";
    }
    ...
    for (x = 0; x < numOfQuestions; x++) {
      ...
      if (tmpObj.image != "no" && tmpObj.image != "") {
        htmlStmt +=
          '<div class="q_img"><img src="' + tmpObj.image + '"></img></div>';
      }
      ...
      if (tmpObj.singleword) {
        ...
        qStr += str.replace(
          /\[_]/g,
          '<input class="text_input_area flexible_answer_input" ' +
            'type="text" maxlength="' +
            tmpObj.maxlength +
            '" ' +
            'data-type="' +
      ...
      if (tmpObj.q_images != undefined && tmpObj.q_images != "") {
        htmlStmt += '<div class="q_images">';
        for (let y = 0; y < tmpObj.q_images.length; y++) {
          htmlStmt += '<img src="' + tmpObj.q_images[y] + '">';
        }
        htmlStmt += "</div>";
      }
  ...
  /*
	تنسيق الإجابات المرنة لهذه الصفحة فقط:
	...
	aren't / are not
*/
  $(".activity_area .flexible_answer_input").each(function () {
    var input = this;

    function normalizeFlexibleAnswer() {
      var value = input.value;

      // حذف الفاصلة وأي علامات ترقيم غير مهمة.
      value = value
        .replace(/[,،.!?؟;:]/g, " ")
        .replace(/\s+/g, " ")
        .trim();

      // تحويل النفي الكامل إلى الاختصار الموجود في answer.
      value = value
        .replace(/\bdoes\s+not\b/gi, "doesn't")
        .replace(/\bdo\s+not\b/gi, "don't")
        .replace(/\bdid\s+not\b/gi, "didn't")
        .replace(/\bis\s+not\b/gi, "isn't")
        .replace(/\bare\s+not\b/gi, "aren't")
        .replace(/\bwas\s+not\b/gi, "wasn't")
        .replace(/\bwere\s+not\b/gi, "weren't")
        .replace(/\bhas\s+not\b/gi, "hasn't")
        .replace(/\bhave\s+not\b/gi, "haven't")
        .replace(/\bhad\s+not\b/gi, "hadn't")
        .replace(/\bcan\s+not\b/gi, "can't")
        .replace(/\bcannot\b/gi, "can't")
        .replace(/\bwill\s+not\b/gi, "won't");

      input.value = value;
    }

    input.addEventListener("input", function () {
      // أثناء الكتابة نحذف الفواصل فقط.
      input.value = input.value.replace(/[,،]/g, " ").replace(/\s+/g, " ");
    });

    input.addEventListener("change", normalizeFlexibleAnswer);
    input.addEventListener("blur", normalizeFlexibleAnswer);
  });

  /*
	قبل الضغط على Check يتم تطبيع الإجابات،
	ثم ملف الـ activity يفحصها بشكل طبيعي دون أي تعديل عليه.
*/
  $(".checkBtn")
    .off("mousedown.flexibleFillin touchstart.flexibleFillin")
    .on("mousedown.flexibleFillin touchstart.flexibleFillin", function () {
      $(".activity_area .flexible_answer_input").each(function () {
        $(this).trigger("change");
      });
    });
