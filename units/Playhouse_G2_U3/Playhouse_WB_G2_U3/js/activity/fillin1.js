...
FillIn.prototype = {
  ...
  validate: function () {
    ...
    function normalizeAnswer(value, strictCase) {
      ...
      return (
        value
          ...
          // تجاهل الفراغات وعلامات الترقيم
          .replace(/[\s,،﹐﹑，､٫.!?؟;:]/g, "")
      );
    }
    ...
    for (var i = 0; i < elsQue.length; i++) {
      ...
      /*
      إذا answer = [""]
      فهذا سؤال مفتوح / حسب رأي الطالب
    */
      var isOpenAnswer =
        Array.isArray(questionData.answer) &&
        questionData.answer.length > 0 &&
        String(questionData.answer[0]).trim() === "";

      if (isOpenAnswer) {
        // ينحسب صح دائمًا
        isCorrect = true;

        // لا نظهر لا صح ولا خطأ
        ...
        readonly يعتبر صحيح
      */
        if (inputBoxes.length > 0 && inputBoxes[0].hasAttribute("readonly")) {
          isCorrect = true;
        }
      ...
      if (isCorrect) {
        ...
        // نظهر الصح فقط إذا مش سؤال مفتوح
        if (!isOpenAnswer) {
          if (tick) {
