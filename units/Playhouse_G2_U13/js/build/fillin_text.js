function buildFillInBody(aObj) {
  ...
  if (typeof aObj != undefined && aObj != null) {
    ...
    for (x = 0; x < numOfQuestions; x++) {
      ...
      var hasInput = tmpObj.text.includes("[_]");
      ...
      if (hasInput) {
        ...
        htmlStmt +=
          '<div class="text_only_row img_fillin_gr d-flex flex-column">';
      }
    ...
    if (aObj.bottomQuestion !== undefined && aObj.bottomQuestion !== null) {
      htmlStmt += '<div class="activityHeading bottom_activity_heading">';

      htmlStmt +=
        '<div class="audioIcon off contant audioQuestionTitle" ' +
        'data-slideNum="1" ' +
        'data-audio="' +
        aObj.bottomQuestion.subTitleAudio +
        '">';

      htmlStmt += '<div class="page_sub_title d-flex">';

      htmlStmt += "<p>" + aObj.bottomQuestion.subTitleTextLeft + "</p>";

      if (
        aObj.bottomQuestion.subTitleIcons !== undefined &&
        aObj.bottomQuestion.subTitleIcons.length > 0
      ) {
        for (
          var bottomIcon = 0;
          bottomIcon < aObj.bottomQuestion.subTitleIcons.length;
          bottomIcon++
        ) {
          htmlStmt +=
            "<img src='" +
            aObj.bottomQuestion.subTitleIcons[bottomIcon] +
            "'/>";
        }
      }
      ...
      htmlStmt +=
        ...
        aObj.bottomQuestion.subTitleTextRight +
        "</p>";
...
function prepareLetterAnswers(aObj) {
  if (!aObj || !aObj.questions || aObj.questions.length < 7) {
    return;
  }

  /*
        1: Text only
        2: I was not born in Spain. I was born in Brazil.
        3: My parents were born in India.
        4: My sister was born in Australia,
           and my brother was not born in Germany.
        5: We were not born in India.
        6: Where were you born?
           Where were your parents born?
        7: Text only
    */

  aObj.questions[0].answer = [];

  aObj.questions[1].answer = ["was not", "was"];

  aObj.questions[2].answer = ["were"];

  aObj.questions[3].answer = ["was", "was not"];

  aObj.questions[4].answer = ["were not"];

  aObj.questions[5].answer = ["were", "were"];

  aObj.questions[6].answer = [];
}

/* =========================================================
   Accept different apostrophe forms
========================================================= */

function normalizeFillInAnswer(value) {
  if (value == null) {
    return "";
  }
  ...
  var normalizedValue = String(value)
    ...
    // توحيد جميع أشكال الفاصلة العليا
    .replace(/[’‘`´ʼ＇]/g, "'")
  ...
  normalizedValue = normalizedValue
    .replace(/\bwasn\s*'\s*t\b/gi, "was not")
    .replace(/\bwasnt\b/gi, "was not")
    .replace(/\bwas\s+not\b/gi, "was not");
  ...
  normalizedValue = normalizedValue
    .replace(/\bweren\s*'\s*t\b/gi, "were not")
    .replace(/\bwerent\b/gi, "were not")
    .replace(/\bwere\s+not\b/gi, "were not");

  return normalizedValue;
}

/* =========================================================
   Normalize immediately before Check
========================================================= */

function enableFlexibleNegativeAnswers() {
  if (window.flexibleNegativeAnswersEnabled) {
    return;
  }

  window.flexibleNegativeAnswersEnabled = true;

  /*
        Capture = true
        يعني هذا الكود يشتغل قبل validate الموجودة
        داخل ملف الـActivity.
    */
  ...
  document.addEventListener(
    ...
    function (event) {
      ...
      var inputs = document.querySelectorAll(".activity_area .text_input_area");
      ...
      for (var i = 0; i < inputs.length; i++) {
        ...
        inputs[i].value = normalizeFillInAnswer(inputs[i].value);
      }
    },
    true,
  );
