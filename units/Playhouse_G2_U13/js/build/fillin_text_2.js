function buildFillInBody(aObj) {
  ...
  if (typeof aObj != undefined && aObj != null) {
    ...
    for (x = 0; x < numOfQuestions; x++) {
      ...
      if (hasInput) {
        htmlStmt +=
          '<div class="que question_row img_fillin_gr d-flex flex-wrap" ' +
          'data-qno="' +
          (x + 1) +
          '">';
      } else {
        htmlStmt +=
          '<div class="text_only_row question_row img_fillin_gr d-flex flex-wrap">';
      }
...
function normalizeOppositeAnswer(value) {
  return (
    String(value || "")
      .trim()
      .toLowerCase()
...
function enableFlexibleOppositeAnswers() {
  if (window.flexibleOppositeAnswersEnabled) {
    return;
  }

  window.flexibleOppositeAnswersEnabled = true;
  ...
  document.addEventListener(
    ...
    function (event) {
      ...
      for (var i = 0; i < inputs.length; i++) {
        inputs[i].value = normalizeOppositeAnswer(inputs[i].value);
      }

      /*
        نحول الإجابات الموجودة في الداتا أيضًا
        إلى نفس الصيغة قبل validate.
      */
      if (typeof fillin_data !== "undefined" && fillin_data.questions) {
        for (var q = 0; q < fillin_data.questions.length; q++) {
          var answers = fillin_data.questions[q].answer;

          if (Array.isArray(answers)) {
            for (var a = 0; a < answers.length; a++) {
              answers[a] = normalizeOppositeAnswer(answers[a]);
            }
          }
        }
      }
