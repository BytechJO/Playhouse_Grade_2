...
function normalizeFillAnswer(value) {
  return String(value || "")
    .toLowerCase()
    .replace(/[’‘`´]/g, "'") // يوحّد أنواع apostrophe
    ...
    for (var i = 0; i < elsQue.length; i++) {
      ...
      if (_uAns.length > 0 && _cAns.length == _uAns.length) {
        for (var cc = 0; cc < _cAns.length; cc++) {
          var userAnswer = normalizeFillAnswer(_uAns[cc]);
          var correctAnswer = normalizeFillAnswer(_cAns[cc]);

          var isCorrect = userAnswer === correctAnswer;

          /* فحص الإجابات البديلة */
          if (!isCorrect && Array.isArray(fDataObj.alternateanswer)) {
            var alternateAnswers = fDataObj.alternateanswer[cc] || [];

            for (var alt = 0; alt < alternateAnswers.length; alt++) {
              if (userAnswer === normalizeFillAnswer(alternateAnswers[alt])) {
                isCorrect = true;
                break;
              }
            }
