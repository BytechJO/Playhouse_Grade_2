    ...
    for (var i = 0; i < elsQue.length; i++) {
      ...
      var _case =
        fDataObj.strictcase != undefined && fDataObj.strictcase != null
          ...
          : "no";
      var _cAns = Array.isArray(fDataObj.answer)
        ? fDataObj.answer.slice()
        : [fDataObj.answer];
      var _uAns = [];
      var _isReadOnly = [];
      var _corr = 0;
      ...
      if (_uAns.length > 0 && _cAns.length == _uAns.length) {
        for (var cc = 0; cc < _cAns.length; cc++) {
          ...
          _uAns[cc] = _case == "yes" ? _uAns[cc] : _uAns[cc].toLowerCase();

          /*
           * تجاهل المسافات والنقطة الموجودة في نهاية الجملة فقط.
           * يعني الجملة تُقبل سواء كتب الطالب النقطة أو لم يكتبها.
           */
          _cAns[cc] = _cAns[cc]
            .trim()
            .replace(/\.\s*$/, "")
