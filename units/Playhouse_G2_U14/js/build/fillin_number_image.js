function buildFillInBody(aObj) {
  ...
  if (typeof aObj != undefined && aObj != null) {
    ...
    // ================= Story box =================
    if (
      aObj.storySentences != undefined &&
      aObj.storySentences != null &&
      aObj.storySentences.length > 0
    ) {
      htmlStmt += '<div class="story_wrapper">';

      htmlStmt += '<button type="button" class="story_toggle">';
      htmlStmt += "<span>Story</span>";
      htmlStmt += '<span class="story_arrow">▼</span>';
      htmlStmt += "</button>";

      htmlStmt += '<div class="story_box">';

      jQuery.each(aObj.storySentences, function (index, sentence) {
        htmlStmt += '<div class="story_sentence">';
        htmlStmt += '<span class="story_number">' + (index + 1) + "</span>";
        htmlStmt += '<span class="story_text">' + sentence + "</span>";
        htmlStmt += "</div>";
      });
    ...
    for (x = 0; x < numOfQuestions; x++) {
      ...
      htmlStmt +=
        '<div class="que img_fillin_gr d-flex flex-column background_audio" data-qno="' +
        (x + 1) +
        '">';
      htmlStmt +=
  ...
  $(".story_toggle")
    ...
    .on("click", function () {
      var $button = $(this);
      var $storyBox = $button.next(".story_box");

      $storyBox.stop(true, true).slideToggle(250);
      $button.toggleClass("open");

      if ($button.hasClass("open")) {
        $button.find(".story_arrow").text("▲");
      } else {
        $button.find(".story_arrow").text("▼");
      }
    });
