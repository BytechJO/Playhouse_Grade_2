...
MCQ.prototype = {
  ...
  validate: function () {
    ...
    for (var i = 0; i < elsQue.length; i++) {
      ...
      if (ob.data_obj.defaultAnswer == fIndx) {
        resultArr[i] = 1;
      ...
      if (inputBoxes.length == 1) {
        var userFillAns = inputBoxes[0].value
          .toLowerCase()
          .trim()
          .replace(/\.\s*$/, "")
          .replace(/\s+/g, " ");

        var corrFillAns = fDataObj.fillinanswer
          .toLowerCase()
          .trim()
          .replace(/\.\s*$/, "")
          .replace(/\s+/g, " ");
  ...
  showIcons: function (aBoo, aVal, aResult) {
    ...
    for (var i = 0; i < elsQue.length; i++) {
      ...
      if (ob.data_obj.defaultAnswer == qNo) {
  ...
  reset: function () {
    ...
    for (var i = 0; i < elsQue.length; i++) {
      var fIndx = parseInt(elsQue[i].dataset.qno);
      var fDataObj = ob.data_obj.questions[fIndx - 1];
      var inputBox = elsQue[i].querySelector("input");

      if (inputBox) {
        if (ob.data_obj.defaultAnswer == fIndx) {
          inputBox.value = fDataObj.fillinanswer;
